...
make_autoplot_figure <- function(logfile, tarmetafile) {
  ...
  df_labels <- df_targets |> 
    ...
    dplyr::mutate(
      n = 1:dplyr::n(),
      resident = (max(df_autometric$resident) / 2) + 0.2 * n,
      name = dplyr::case_match(
        name,
        "individual_splsda_perf_rnaseq" ~ "sPLS-DA prefiltering (RNAseq)",
        "individual_splsda_perf_snps" ~ "sPLS-DA prefiltering (SNPs)",
        "spls_tune_res" ~ "sPLS cross-validation tuning",
        "diablo_tune_res" ~ "DIABLO cross-validation tuning"
      ),
...
make_figure_runtime_genomics <- function(logfile, tarmetafile) {
  ...
  df_labels <- df_targets |> 
    ...
    dplyr::mutate(
      n = 1:dplyr::n(),
      resident = (max(df_autometric$resident) / 3) + 0.4 * n,
      name = dplyr::case_match(
        name,
        "full_individual_splsda_perf" ~ "sPLS-DA prefiltering (full dataset)",
        "reduced_individual_splsda_perf" ~ "sPLS-DA prefiltering (reduced dataset)",
        "full_pca_runs_list" ~ "NIPALS- PCA on full dataset"
      ),
      name = stringr::str_wrap(name, width = 10)
    )
